...
st.title("📈 賊大選股")
st.caption("核心選股版 v1.3｜新增策略勝率＋每檔入選原因")
...
if d is not None and not d.empty:
    ...
    with tabs[0]:
        ...
        show = x[base_cols()+["r20","turnover_pct","reason1"]].sort_values("r20", ascending=False)
        show.columns = ["代號","名稱","市場","收盤","今日量(張)","5日均量(張)","股本(億)","20日漲幅%","週轉率%","入選原因"]
        st.dataframe(show, use_container_width=True, hide_index=True)

    with tabs[1]:
        x = d[d["cond2"].fillna(False)].copy()
        show = x[base_cols()+["volume_vs_5avg_pct","r30","reason2"]].sort_values("volume_vs_5avg_pct", ascending=False)
        show.columns = ["代號","名稱","市場","收盤","今日量(張)","5日均量(張)","股本(億)","量/5日均量%","30日漲幅%","入選原因"]
        st.dataframe(show, use_container_width=True, hide_index=True)
        st.caption("課程寫『再搭配外資跟投信的動態來判斷』；此欄目前先保留為二次人工判讀，不把它偷改成硬門檻。")

    with tabs[2]:
        x = d[d["cond3"].fillna(False)].copy()
        show = x[base_cols()+["r10","volume_vs_5avg_pct","reason3"]].sort_values("volume_vs_5avg_pct", ascending=False)
        show.columns = ["代號","名稱","市場","收盤","今日量(張)","5日均量(張)","股本(億)","10日漲幅%","量/5日均量%","入選原因"]
        st.dataframe(show, use_container_width=True, hide_index=True)

    with tabs[3]:
        x = d[d["cond4"].fillna(False)].copy()
        show = x[base_cols()+["ma5","r40","inst10_net_lots","reason4"]].sort_values("r40", ascending=False)
        show.columns = ["代號","名稱","市場","收盤","今日量(張)","5日均量(張)","股本(億)","5MA","40日漲幅%","10日三大法人淨買賣(張)","入選原因"]
        st.dataframe(show, use_container_width=True, hide_index=True)
    ...
    with tabs[4]:
        ...
        show = x[base_cols()+["eps3_sum","revenue_yoy","revenue_high","reason5"]].sort_values("revenue_yoy", ascending=False)
        show.columns = ["代號","名稱","市場","收盤","今日量(張)","5日均量(張)","股本(億)","近三季EPS合計","營收成長率%","營收歷史新高","入選原因"]
        st.dataframe(show, use_container_width=True, hide_index=True)
